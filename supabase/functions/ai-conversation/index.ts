...
serve(async (req) => {
  ...
  try {
    ...
    // Parse request body
    const { message, conversationHistory, regenerateSection, currentMoodboard }: ConversationRequest = await req.json()
    ...
    // Check generation limits (skip for premium users)
    if (typedUserLimits.subscription_status !== 'premium' && 
...
CRITICAL: You MUST respond with valid JSON in this exact format:
{
  ...
  "isComplete": true
    ...
    if (regenerateSection && currentMoodboard) {
      prompt += `\n\nCurrent design system context:\n${JSON.stringify(currentMoodboard, null, 2)}\n\nUser request: Regenerate the ${regenerateSection} section. ${message}`
    } else {
    ...
    const generationsRemaining = Math.max(0, typedUserLimits.generation_limit - (typedUserLimits.current_generation_count + 1))
    const tomorrow = new Date()
    tomorrow.setUTCDate(tomorrow.getUTCDate() + 1)
    tomorrow.setUTCHours(0, 0, 0, 0)
    ...
    const response: ConversationResponse = {
      ...
      usage: {
        ...
        generationsRemaining: typedUserLimits.subscription_status === 'premium' ? -1 : generationsRemaining,
        resetTime: tomorrow.toISOString()
      }
    }
